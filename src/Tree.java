...
public class Tree {
  // on crée l'abre au fur et à mésure
  ...
  private static Triplet minBlue(Triplet leftRes, Triplet rightRes) {
    if(leftRes.getMinBlue() < rightRes.getMinBlue()) return new Triplet(false, rightRes.getMinBlue(), rightRes.getMinOrange());
    else  {
      return  new Triplet(true, leftRes.getMinBlue(), leftRes.getMinBlue());
    }
  ...
  private static Triplet minOrange(Triplet leftRes, Triplet rightRes) {
    ...
    else return  new Triplet(true, leftRes.getMinBlue(), leftRes.getMinOrange());
  }
  ...
  public void computeMinimaxValues() {
    if(this.state.getLeftBar() == state.getRightBar()){
      new Triplet(true,state.getBluePoints(),state.getOrangePoints());
    }else {
